...
const InventoryPage = () => {
  ...
  const fetchInventoryData = async () => {
    try {
      const [summaryRes, alertsRes, analysisRes, valuationRes] = await Promise.all([
        fetch('/api/inventory', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }),
        fetch('/api/inventory/alerts', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }),
        fetch('/api/inventory/analysis', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }),
        fetch('/api/inventory/valuation', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
      ]);
