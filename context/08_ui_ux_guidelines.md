...
Bu doküman, AI Researcher Portfolio projesinin görsel tasarımını, etkileşim prensiplerini ve kullanıcı deneyimi hedeflerini belirlemektedir. Amaç, sitenin estetik, tutarlı, sezgisel ve erişilebilir bir arayüze sahip olmasını sağlamaktır.
...
*   **Ana Renkler (Primary):**
    *   **Koyu Mavi (Arka Plan):** `#0A192F` - Sitenin genel arka planı. Ciddi ve odaklanmış bir atmosfer yaratır.
    *   **Açık Gri (Metin):** `#CCD6F6` - Ana metin rengi. Koyu arka plan üzerinde yüksek okunabilirlik sağlar.
    *   **Beyaz (Başlıklar/Vurgular):** `#FFFFFF` - Ana başlıklar ve önemli vurgular için. Kontrast yaratır.

*   **Vurgu Rengi (Accent):**
    *   **Mint Yeşili (Etkileşim):** `#64FFDA` - Linkler, butonlar, aktif menü öğeleri ve diğer etkileşimli elemanlar için kullanılacak ana vurgu rengi. Enerjik ve modern bir dokunuş sağlar.

*   **Nötr Renkler (Neutral):**
    *   **Açık Mavi (İkincil Metin):** `#8892B0` - İkincil metinler, açıklamalar ve meta veriler için.
    *   **Koyu Gri (Kart Arka Planı):** `#112240` - Proje kartları gibi bileşenlerin arka planı için. Ana arka plandan hafifçe ayrışır.
